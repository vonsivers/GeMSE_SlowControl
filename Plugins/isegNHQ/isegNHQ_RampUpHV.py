...
while True:
    key=raw_input("Are you sure you want to RAMP UP the HV?\ny/n?")
    if key=='n':
        exit()
    elif key=='y':
        break
...
# set voltage to 40V
print "setting voltage to 4000V ..."
WriteRead(ser,'D1=4000\r\n')
 
# check if parameter was set correctly
response=WriteRead(ser,'D1\r\n')
if float(response)==4000:
    print "voltage set"
else:
...
if response=='S1=L2H\r\n':
    ...
    print "ERROR ramping up voltage!"
    print "response of isegNHQ:", response
    ser.close()
    exit()
