...
class MKS_MFCSetValvePosition(object):
    """
    ...
    def valveposition(self):
        """
        Manually changes the valve position.
        """
        try:
            print("\nReading current valve position...")
            old_position = self.controller.getValvePosition()
            print("The current valve position is %s" % (
                str(old_position)))
            text = "Enter the new valve position (n: normal, p: purge, c: closed):" 
            input_string = self.__getUserInput__(text, input_type=[str], be_in=['n','p','c'])
            if input_string=='p':
                new_position="PURGE"
            elif input_string=='n':
                new_position="NORMAL"
            else:
                new_position="FLOW_OFF"
            print "Trying to change the position to %s..." % str(new_position)
            if self.controller.changeValvePosition(new_position) != new_position:
                print("    Failed to change the valve position.")
            print("Sucessfully changed valve position from %s to %s." %
                  (str(old_position), str(new_position)))
            print("Get confirmation...")
        except KeyboardInterrupt:
            print("Valve positon change aborted! Reading current valve position...")
        finally:
            confirmation = self.controller.getValvePosition()
            if confirmation != -1:
                print("The valve position is %s." % str(confirmation))
            else:
                print("Can not confirm current valve position.")
...
if __name__ == '__main__':
    ...
    try:
        ...
        self.logger.warning("Can not add path '%s/Plugins/MKS_MFC/'. "
                            "Error %s." % (opts.path, e))
    MKS_MFC_setposition = MKS_MFCSetValvePosition(opts)
    MKS_MFC_setposition.valveposition()
    MKS_MFC_setposition.close()
    sys.exit(0)
